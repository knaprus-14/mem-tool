...
func TestTUICommandMenuCoversCLICommandsAndSubcommands(t *testing.T) {
	...
	for _, command := range []string{
		"init", "version", "help", "open", "add", "search", "ask", "map", "mindmap",
	...
	for _, subcommand := range []string{
		...
		"map build", "map coverage", "map eval", "map eval-labeled", "map profile", "map diff", "map extract", "map extract-runs", "map extract-run",
