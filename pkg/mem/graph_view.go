...
func (s *Store) BuildKnowledgeMapViewData() (KnowledgeMapViewData, error) {
	...
	return KnowledgeMapViewData{
		...
		Graph: graph, Review: review, Merges: merges,
	}, nil
}
...
button,input{font:inherit;color:inherit}button{background:#171c24;border:1px solid #384250;padding:6px 9px;cursor:pointer}button:hover{border-color:#7f8b9a}.app{height:100%;display:grid;grid-template-columns:260px minmax(320px,1fr) 340px;grid-template-rows:48px 1fr}
...
<header><div class="brand">MEM / KNOWLEDGE MAP</div><div id="summary" class="summary"></div><div class="tools"><button id="filtersBtn">FILTERS</button><button id="minusBtn">−</button><button id="fitBtn">FIT</button><button id="plusBtn">+</button><button id="detailsBtn">DETAILS</button></div></header>
<aside id="filters" class="filters"><input id="search" class="search" placeholder="Поиск /" autocomplete="off"><div id="stats" class="stat-grid"></div><h2>Статусы</h2><div id="statusFilters" class="filter-list"></div><h2>Evidence</h2><div id="evidenceFilters" class="filter-list"></div><h2>Типы узлов</h2><div id="kindFilters" class="filter-list"></div><h2>Связи</h2><div id="relationFilters" class="filter-list"></div><div class="footer-note">Колесо — масштаб · drag фона — pan · drag узла — перемещение · / — поиск · Esc — сброс выбора</div></aside>
...
let W=1,H=1,scale=1,tx=0,ty=0,selected=null,drag=null,pan=null,running=true;
...
function seed(){const radius=Math.max(120,Math.min(W,H)*.34);nodes.forEach((n,i)=>{const a=(i/Math.max(1,nodes.length))*Math.PI*2;n._x=W/2+Math.cos(a)*radius;n._y=H/2+Math.sin(a)*radius})}
...
function badge(value){const b=document.createElement('span');b.className='badge '+value;b.textContent=value;return b}
function showEvidence(item){const wrap=document.createElement('div');const r=reviewFor(item.from?'edge':'node',item.id);for(const ev of(r?.evidence||[])){const box=document.createElement('div');box.className='evidence';const badges=document.createElement('div');badges.className='badges';badges.append(badge(ev.state));const coord=document.createElement('div');coord.className='coordinates';const a=ev.anchor;coord.textContent=a.source_path+' · page '+(a.page||'?')+' · block '+a.block_index+' · chunk '+a.block_chunk_index;const ex=document.createElement('div');ex.className='excerpt';ex.textContent=a.excerpt;const h=document.createElement('div');h.className='hash';h.textContent='citation '+a.citation_id+'\nrevision '+a.document_revision+'\nchunk '+a.chunk_hash+'\nevidence '+a.evidence_hash;box.append(badges,coord,ex,h);wrap.append(box)}return wrap}
function selectObject(type,item){selected={type,item};document.querySelectorAll('.selected').forEach(x=>x.classList.remove('selected'));if(type==='node')item._g.classList.add('selected');else item._line.classList.add('selected');details.textContent='';const title=document.createElement('div');title.className='detail-title';title.textContent=item.label||item.id;const badges=document.createElement('div');badges.className='badges';for(const v of[type,item.kind,item.status,item.origin,evidenceState(type,item.id)])if(v)badges.append(badge(v));details.append(title,badges,field('ID',item.id));if(type==='edge')details.append(field('Endpoints',item.from+' → '+item.to));if(item.body)details.append(field('Body',item.body));details.append(field('Confidence',item.confidence||0),field('Updated',item.updated));const merge=mergeBySource.get(item.id);if(merge){const m=document.createElement('div');m.className='field merge';m.append(field('Merged into',merge.target_id),field('Reviewer',merge.reviewer),field('Similarity',merge.similarity));details.append(m)}details.append(field('Evidence digest',reviewFor(type,item.id)?.evidence_digest||''),showEvidence(item));details.classList.remove('closed')}
function point(ev){const r=svg.getBoundingClientRect();return{x:ev.clientX-r.left,y:ev.clientY-r.top}}
function startNodeDrag(ev,n){ev.stopPropagation();const p=point(ev);drag={n,dx:(p.x-tx)/scale-n._x,dy:(p.y-ty)/scale-n._y};n._fixed=true;svg.setPointerCapture(ev.pointerId)}
svg.addEventListener('pointerdown',ev=>{if(ev.target===svg){const p=point(ev);pan={x:p.x-tx,y:p.y-ty};svg.setPointerCapture(ev.pointerId)}});svg.addEventListener('pointermove',ev=>{const p=point(ev);if(drag){drag.n._x=(p.x-tx)/scale-drag.dx;drag.n._y=(p.y-ty)/scale-drag.dy}else if(pan){tx=p.x-pan.x;ty=p.y-pan.y}});svg.addEventListener('pointerup',()=>{if(drag){drag.n._fixed=false;drag=null}pan=null});svg.addEventListener('click',()=>{selected=null;document.querySelectorAll('.selected').forEach(x=>x.classList.remove('selected'))});
function zoomAt(f,cx=W/2,cy=H/2){const old=scale;scale=Math.max(.15,Math.min(4,scale*f));tx=cx-(cx-tx)*(scale/old);ty=cy-(cy-ty)*(scale/old)}svg.addEventListener('wheel',ev=>{ev.preventDefault();const p=point(ev);zoomAt(ev.deltaY<0?1.12:.89,p.x,p.y)},{passive:false});
...
$('plusBtn').onclick=()=>zoomAt(1.25);$('minusBtn').onclick=()=>zoomAt(.8);$('fitBtn').onclick=fit;$('detailsBtn').onclick=()=>details.classList.toggle('closed');$('filtersBtn').onclick=()=>$('filters').classList.toggle('closed');search.addEventListener('input',applyFilters);document.addEventListener('keydown',ev=>{if(ev.key==='/'&&document.activeElement!==search){ev.preventDefault();search.focus()}if(ev.key==='Escape'){search.value='';applyFilters();selected=null;document.querySelectorAll('.selected').forEach(x=>x.classList.remove('selected'))}});
new ResizeObserver(()=>{const r=svg.getBoundingClientRect(),first=W===1;W=Math.max(1,r.width);H=Math.max(1,r.height);if(first){seed();setTimeout(fit,350)}}).observe(svg);buildFilters();initGraph();tick();
})();
</script>
</body>
</html>
