...
func tuiCommandMenuItems() []commandMenuEntry {
	return []commandMenuEntry{
		...
		{"mindmap open [--no-browser]", "открыть библиотеку и редактор классических карт"},
		...
		{"mindmap source-add <карта> <узел> --entry N|--file <путь>|--url <адрес>|--knowledge-node <ID>", "привязать фрагмент, файл, ссылку или узел графа"},
		{"mindmap source-remove <карта> <узел> --source <ID>", "отвязать источник без удаления исходных данных"},
		{"mindmap source-move <карта> <узел> --source <ID> --position N", "изменить порядок источников узла"},
		...
		{"mindmap templates [--json]", "показать встроенные шаблоны классических карт"},
		{"mindmap template-create <шаблон> <название> [--json]", "создать ручную карту из шаблона"},
		{"mindmap ask-branch <карта> <узел> <вопрос> [--json]", "задать grounded-вопрос по evidence ветви"},
		{"mindmap compare <левая> <правая> [--json]", "сравнить два полных дерева локально"},
		{"mindmap study <карта> <узел> [--output файл] [--force] [--json]", "собрать детерминированный учебный набор"},
		{"mindmap export <карта> --format html|svg|png|json|opml|markdown|mermaid|obsidian --output <путь> [--force]", "создать переносимый снимок полного дерева"},
		...
		{"map profile [-iterations N] [--view <имя>] [--json]", "измерить SQLite, evidence, view и HTML стадии"},
