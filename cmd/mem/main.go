...
func handleMap(cfg *Config, store *Store, args []string) error {
	...
	default:
		return fmt.Errorf("неизвестная подкоманда map: %s (доступны open, build, coverage, eval, diff, snapshots, corpus-diff, restore, restore-runs, extract, extract-runs, extract-run, analyze, duplicates, merge-node, merges, runs, run, prune-runs, status, approve, approve-batch, reviews, edits, export, export-html)", args[0])
	}
}
...
Использование:
  ...
  mem map eval <manifest.json> [--json]
      Применить строгий воспроизводимый quality gate к текущему coverage-снимку.
      Manifest задаёт минимальные проценты импорта/обработки/покрытия и пределы
      OCR warnings, draft, stale/missing и других счётчиков. Модель не вызывается;
      непройденный gate печатает полный отчёт и завершает команду с кодом 1.
